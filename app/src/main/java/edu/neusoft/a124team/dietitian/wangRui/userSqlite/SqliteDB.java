package edu.neusoft.a124team.dietitian.wangRui.userSqlite;
        ...
        if (user != null) {
           /* ContentValues values = new ContentValues();
            ...
            if (cursor.getCount() > 0) {
                ...
                try {
                    db.execSQL
                            ("insert into User(username,userpwd) values(?,?) ", new String[]
                                    {user.getUsername().toString(), user.getUserpwd().toString()});
                } catch (Exception e) {
...
/*       if (cursor.getCount()>0)
        {
            Cursor pwdcursor =db.rawQuery
                    ("select * from User where userpwd=? and username=?",new String[]{pwd,name});
